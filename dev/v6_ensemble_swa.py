...
def train_full(args):
    ...
    # define swa:
    opt1_swa = SWA(optimizer1, swa_start=int(0.75*args.epochs), swa_freq=5, swa_lr=0.05)
    opt2_swa = SWA(optimizer2, swa_start=int(0.75*args.epochs), swa_freq=5, swa_lr=0.05)
    opt3_swa = SWA(optimizer3, swa_start=int(0.75*args.epochs), swa_freq=5, swa_lr=0.05)
    ...
    v_acc1, net1 = train_swa(epochs=args.epochs, network=net1, criterion=criterion, train_loader=train_loader, optimizer=optimizer1, scheduler=scheduler1, val_loader=val_loader, network_flag=1)
    v_acc2, net2 = train_swa(epochs=args.epochs, network=net2, criterion=criterion, train_loader=train_loader, optimizer=optimizer2, scheduler=scheduler2, val_loader=val_loader, network_flag=2)
    v_acc3, net3 = train_swa(epochs=args.epochs, network=net3, criterion=criterion, train_loader=train_loader, optimizer=optimizer3, scheduler=scheduler3, val_loader=val_loader, network_flag=3)
