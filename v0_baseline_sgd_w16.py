...
def main(args):
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            self.w = 48
            self.drop = 0.7
            self.conv1 = nn.Conv2d(1, self.w, kernel_size=5)
            self.conv2 = nn.Conv2d(self.w, self.w, kernel_size=5)
            self.conv3 = nn.Conv2d(self.w, self.w*2, kernel_size=5)
            self.fc1 = nn.Linear(3 * 3 * self.w * 2, 1024)
